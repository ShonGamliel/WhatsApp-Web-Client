...
import { userProvider, currentChatProvider, chatsHistoryProvider } from "../../App";
import React from "react";
import { send } from "../../server/WebSocket";
import { getChatID } from "../../functions/Functions";

export default function ChatInput({ newMessages, setNewMessages }) {
  const { user } = React.useContext(userProvider);
  const { currentChat } = React.useContext(currentChatProvider);
  const { updateChatsHistory } = React.useContext(chatsHistoryProvider);
          ...
          if (text.length != 0) {
            if (currentChat) {
              let messageToSend = {
                ...
                timestamp: Date.now(),
                from: user._id,
                to: currentChat.to == user._id ? currentChat.from : currentChat.to,
                fromusername: user.username,
                tousername: currentChat.to == user._id ? currentChat.fromusername : currentChat.tousername,
                chatid: getChatID(user._id, currentChat.to == user._id ? currentChat.from : currentChat.to),
              };
              send(messageToSend);
              updateChatsHistory(messageToSend);
              setNewMessages((newMessages) => newMessages.concat(messageToSend));
              setMessage("");
              document.getElementById("chat-input-text").innerHTML = "";
              send({ typing: false, chatid: getChatID(user._id, currentChat.to == user._id ? currentChat.from : currentChat.to), from: user._id, to: currentChat.to == user._id ? currentChat.from : currentChat.to });
            }
          }
        }}
      >
        ...
        onKeyDown={(e) => {
          ...
          if (e.target.innerText.length == 0) {
            ...
            if (e.target.innerText.length % 5 == 0 || e.target.innerText.length == 1) {
              send({ typing: true, chatid: getChatID(user._id, currentChat.to == user._id ? currentChat.from : currentChat.to), from: user._id, to: currentChat.to == user._id ? currentChat.from : currentChat.to });
            }
          }
        }}
      />
